...
func TestRenderPage_LocaleHTMLAttribute(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.locale, func(t *testing.T) {
			tr, _ := NewTranslator("")
			html, err := RenderPage("test", "./", "./_thumbs/", "", "", false, false, false, 0, []string{"30", "60", "120", "all"}, files, nil, defaultImageExts, defaultVideoExts, defaultVideoExts, "", "", "substring", tc.locale, tr, "00", "00", "00", "00", "00", false, false, "/tmp/dir", "/tmp/cache", "webp")
			if err != nil {
				t.Fatal(err)
			}
...
func TestRenderPage_TranslationMissingFallback(t *testing.T) {
	...
	html, err := RenderPage("test", "./", "./_thumbs/", "", "", false, false, false, 0, []string{"30", "60", "120", "all"}, files, nil, defaultImageExts, defaultVideoExts, defaultVideoExts, "", "", "substring", "de", tr, "00", "00", "00", "00", "00", false, false, "/tmp/dir", "/tmp/cache", "webp")
	if err != nil {
		t.Fatal(err)
	}
