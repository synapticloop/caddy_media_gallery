...
func (g *Gallery) ServeHTTP(w http.ResponseWriter, r *http.Request, next caddyhttp.Handler) error {
	...
	//
	// Per user request 2026-07-01: enrich the FILE LIST
	...
	// this would be ~5s — too slow. The fix is to limit
	// the sync enrich to the visible page's file set; for
	// now we accept the latency on the first visit (most
	// directories have <500 files, so the request is
	// bounded at ~600ms).
	scanner := &Scanner{
		Root:         resolved,
		Sort:         g.Sort,
	...
	scanner.enrichParallel(files, 8)
	...
	body, err := RenderPage(title, "./", "./_thumbs/", relPath, g.Template, g.NoThumbs, g.NoVideoThumbs, g.NoAudioMeta, g.PageSize, g.PageSizes, files, r.URL.Query(), g.imageExtsMap, g.videoExtsMap, g.audioExtsMap, g.rootName, g.PathPrefix, g.SearchMatch, locale, g.translator, cacheXX, cacheYY, cacheZZ, cacheAA, cacheBB, g.NoExif, g.NoMeta, resolved, g.thumbCacheDir(), g.ThumbFormat)
	if err != nil {
