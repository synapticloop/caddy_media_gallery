...
#                               # PORT defaults to 3245 (must be > 1024 to avoid needing root).
#                               # 3245 = 0xCAD in hex (C-A-D happen to all be valid
#                               # hex digits, so the abbreviation is memorable).
...
# Per user request 2026-07-01: default the local-install port
# to 3245 instead of 8080. 3245 = 0xCAD in hex — a small easter
# egg for the project's homepage. Override with
# CADDY_USER_PORT=PORT (or --user PORT).
USER_PORT="${CADDY_USER_PORT:-3245}"
...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "               serves CADDY_USER_ROOT (default ~/Pictures) on PORT (default 3245 = 0xCAD)."
            echo ""
            echo "Env vars (--user only):"
            echo "  CADDY_USER_PORT  Port to listen on (default 3245 = 0xCAD; must be > 1024 to avoid needing root)"
...
# The default port is 3245 (0xCAD in hex) — override via CADDY_USER_PORT
