...
func TestScanCache_ReusesOnNoChange(t *testing.T) {
	...
	first, err := c.Get(dir, "mtime", defaultImageExts, defaultVideoExts)
	if err != nil {
		t.Fatal(err)
	}
	second, err := c.Get(dir, "mtime", defaultImageExts, defaultVideoExts)
	if err != nil {
		t.Fatal(err)
	}
...
func TestScanCache_RefreshesOnMtimeChange(t *testing.T) {
	...
	first, _ := c.Get(dir, "mtime", defaultImageExts, defaultVideoExts)
	if len(first) != 1 {
	...
	second, _ := c.Get(dir, "mtime", defaultImageExts, defaultVideoExts)
	if len(second) != 2 {
...
func TestScanCache_DifferentSortCachesSeparately(t *testing.T) {
	...
	c := NewScanCache(time.Minute)
	byMtime, _ := c.Get(dir, "mtime", defaultImageExts, defaultVideoExts)
	byName, _ := c.Get(dir, "name", defaultImageExts, defaultVideoExts)
...
func TestScanCache_BadDirReturnsError(t *testing.T) {
	c := NewScanCache(time.Minute)
	if _, err := c.Get("/this/does/not/exist", "mtime", defaultImageExts, defaultVideoExts); err == nil {
