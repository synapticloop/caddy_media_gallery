...
func TestScanCache_ReusesOnNoChange(t *testing.T) {
	...
	first, err := c.Get(dir, "mtime")
	if err != nil {
		t.Fatal(err)
	}
	second, err := c.Get(dir, "mtime")
	if err != nil {
		t.Fatal(err)
	}
...
func TestScanCache_DifferentSortCachesSeparately(t *testing.T) {
	...
	c := NewScanCache(time.Minute)
	byMtime, _ := c.Get(dir, "mtime")
	byName, _ := c.Get(dir, "name")
...
func TestScanCache_BadDirReturnsError(t *testing.T) {
	c := NewScanCache(time.Minute)
	if _, err := c.Get("/this/does/not/exist", "mtime"); err == nil {
