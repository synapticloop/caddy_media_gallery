...
| `audio_types` | space-separated extensions (no leading dot) | `` (empty — audio opt-in) | Audio file extensions to recognize as `KindAudio`. Per user request 2026-07-04 (1.1.0+): audio support is opt-in — the default list is empty, so existing 1.0.x configurations are unaffected (no audio files recognised, no audio filter, no audio meta panel). When the operator sets `audio_types .mp3 .flac .opus`, files matching those extensions become `KindAudio`, get an SVG speaker-icon tile placeholder, a `<audio controls>` element in the lightbox, and stream-level metadata (codec, sample rate, channels, channel layout, duration, bitrate) extracted via ffprobe (cached in `.ameta` sidecars). Same shape as `image_types` / `video_types`. Per Q2 ("video wins"): files whose extension is in BOTH `audio_types` and `video_types` (e.g. .ogg) are classified as `KindVideo` at extension-only classification time. Recommended starting list: `.mp3 .m4a .aac .flac .opus .wav .ogg .oga`. See [docs/03i-feature-audio.md](03i-feature-audio.md) for the full architecture. |
| `no_audio_meta` | `true` / `false` (no-arg = `true`) | `false` (audio meta on by default when `audio_types` is set) | Disable audio metadata extraction entirely (codec, sample rate, channels, channel layout, duration, bitrate via ffprobe). Per user request 2026-07-04 (1.1.0+): defaults to FALSE (audio metadata extraction on by default when `audio_types` is enabled) — unlike `no_meta` which defaults to `true` (no pre-1.1 audio metadata behaviour to be backward-compatible with). When set, the scanner skips `readAudioMetaCached` for `KindAudio` files (no ffprobe subprocess, no `.ameta` sidecar writes, no parsing). The file is still recognised as `KindAudio` (the SVG tile + lightbox audio player still work) — only the audio metadata card in the lightbox is missing fields. |
...
| `audio_types <list>` | `"audio_types"` | `[]string` | (empty — audio opt-in) |
| `no_audio_meta` / `no_audio_meta false` | `"no_audio_meta"` | bool | `false` |
