...
| `no_exif` | `true` / `false` (no-arg = `true`) | `false` (EXIF on) | Skip EXIF metadata reading for image files. The "EXIF" pill on each card and the EXIF panel in the lightbox are hidden (they only render when `FileInfo.Exif` is non-nil). Useful for privacy-sensitive deployments or to skip the per-image EXIF read (~1-5ms each). Note that EXIF does NOT include GPS by default — see the EXIF section for details. |
