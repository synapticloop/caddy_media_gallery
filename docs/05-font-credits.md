...
This document is set in **Libre Baskerville** (body text) and
**JetBrains Mono** (code blocks). Both are SIL Open Font
License 1.1 — free for commercial and personal use, with
attribution.

## Libre Baskerville

Copyright © 2012 Pablo Impallari, Igino Marini (K-Type).
Based on the Baskerville typeface designed by John Baskerville
in the 1750s.

SIL Open Font License, Version 1.1 (the "License"). A copy of
the License may be obtained at:

  https://scripts.sil.org/OFL

This Font Software is licensed under the SIL Open Font License,
Version 1.1. The full license text follows below; it is also
available with a FAQ at:

  https://scripts.sil.org/OFL/faq
...
AND NONINFRINGEMENT OF COPYRIGHT, PATENT, TRADEMARK, AND OTHER
RIGHTS. THE COPYRIGHT HOLDER WILL NOT BE LIABLE FOR ANY CLAIM
...
SIL Open Font License, Version 1.1 (the same license as above —
the full license text is reproduced under "Libre Baskerville"
above; the OFL text is identical).

Project page: <https://www.jetbrains.com/lp/mono/>

## Bundled location

The TTF files for both fonts are bundled in this repository at
`docs/fonts/`:

| File | Family | Style |
|------|--------|-------|
| `LibreBaskerville-VF.ttf` | Libre Baskerville | Regular (variable weight 400-700) |
| `LibreBaskerville-Italic-VF.ttf` | Libre Baskerville | Italic (variable weight 400-700) |
| `JetBrainsMono-VF.ttf` | JetBrains Mono | Regular (variable weight 100-800) |
| `JetBrainsMono-Italic-VF.ttf` | JetBrains Mono | Italic (variable weight 100-800) |

The `VF` suffix indicates the variable-font axis. Both fonts
include the full weight range in a single file, so a single
fontset declaration in the LaTeX preamble handles all weights
without needing separate files for bold/italic.

## How the fonts are loaded

The fonts are loaded by `docs/preamble.tex` (the LaTeX preamble
included in every page of this PDF) via `\setmainfont` (for
Libre Baskerville) and `\setmonofont` (for JetBrains Mono),
both with a relative `Path = ./fonts/` so the preamble is
portable across users (no hardcoded `/home/<user>/...` paths
in the committed file).

If you want to replace the fonts with alternatives:

1. Drop your TTF/OTF files in `docs/fonts/` (using the same
   filenames so the preamble doesn't need to change), OR
2. Edit `docs/preamble.tex` to point at your font files and
   re-run `./build-docs.sh`.

The bundled fonts are licensed under SIL OFL 1.1 (free for
commercial use, modification, and redistribution). Any
substitution is your responsibility — ensure the replacement
font's license is compatible with the project.