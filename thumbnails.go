...
func evictIfOver(cacheDir string, maxMB int, tracker *cacheStatsTracker) {
	...
	// Record the eviction run for the cache-stats footer.
	// The tracker is nil-tolerant (recordEvictions is a
	// no-op when the receiver is nil), so callers don't
	// need to guard.
	if tracker != nil {
		tracker.recordEvictions(evictedCount, time.Now())
	}
}
