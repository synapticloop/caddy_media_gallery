...
// The remaining segments are the relPath path components.
func TestComputeBreadcrumb(t *testing.T) {
	cases := []struct {
		...
		// root only: breadcrumbRoot is the only segment
		{
		...
		// One level deep: root + one segment
		{
			name:           "one level deep",
			relPath:        "photos/",
			title:          "title-not-used",
		...
		// Real-world case: relPath is "media_gallery/animals/"
		// (Caddy's handle_path strips "/images/" before relPath
		// is computed). breadcrumbRoot is "images" so the
		// breadcrumb shows "images / media_gallery / animals".
		{
			name:           "user-reported case (URL /images/media_gallery/animals/)",
		...
		// breadcrumbRoot empty falls back to title
		{
			name:           "empty breadcrumbRoot falls back to title",
			relPath:        "",
			title:          "Gallery",
			pathPrefix:     "./",
			breadcrumbRoot: "",
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := computeBreadcrumb(c.relPath, c.title, c.pathPrefix, c.breadcrumbRoot)
			if !reflect.DeepEqual(got, c.want) {
