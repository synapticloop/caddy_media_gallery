...
func TestRenderPage_ContainsImagesAndFilenames(t *testing.T) {
	...
	html, err := RenderPage("Test Gallery", "./", "./_thumbs/", "", "", false, files, nil)
	if err != nil {
...
func TestRenderPage_NoOtherFilesSectionWhenEmpty(t *testing.T) {
	...
	html, err := RenderPage("x", "./", "./_thumbs/", "", "", false, files, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderPage_HTMLIsValidish(t *testing.T) {
	...
	html, err := RenderPage("t", "./", "./_thumbs/", "", "", false, files, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderPage_DirectoriesAlwaysRendered(t *testing.T) {
	...
	html, err := RenderPage("test", "./", "./_thumbs/", "", "", false, files, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderPage_PaginationLinksPresent(t *testing.T) {
	...
	// Test page 2
	q := url.Values{"page": {"2"}}
	html2, err := RenderPage("test", "./", "./_thumbs/", "", "", false, files, q)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderPage_SortUITogglesDirection(t *testing.T) {
	...
	html, _ := RenderPage("test", "./", "./_thumbs/", "", "", false, files, nil)
	...
	html, _ = RenderPage("test", "./", "./_thumbs/", "", "", false, files, q)
...
func TestRenderPage_EmptyDirShowsEmptyMessage(t *testing.T) {
	html, err := RenderPage("empty", "./", "./_thumbs/", "", "", false, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderPage_UpEntryInSubdir(t *testing.T) {
	...
	html, err := RenderPage("subdir", "./", "./_thumbs/", "subdir", "", false, files, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRenderPage_NoUpEntryAtRoot(t *testing.T) {
	...
	html, err := RenderPage("root", "./", "./_thumbs/", "", "", false, files, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSplitFiles_DirsUnaffectedByImageSort(t *testing.T) {
	...
	for _, sortSpec := range []string{"mtime", "name", "size"} {
		for _, order := range []string{"asc", "desc"} {
			...
			q.Set("sort", sortSpec)
			q.Set("order", order)
			html, err := RenderPage("test", "./", "./_thumbs/", "", "", false, files, q)
			if err != nil {
