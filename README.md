...
> **Release 1.1.1 — 2026-07-04.** Patch release on the 1.1.x line. Stable; fixes two cosmetic bugs in the cache-status footer: (1) the XX hex value was scaled to 0-100 instead of 0-255, so 100% showed as 0x64 instead of 0xFF; (2) the footer rendered in the proportional body font because `--font-mono` was referenced but never defined, so the digit pairs were slightly misaligned. Both fixed: the function is now `CacheUsageFractionHex255` (returning 0-255) and the CSS variable is now defined. Operators can pin to `1.1.1` (or any later version) in their build pipeline.
...
- **Audio support (1.1.0+, opt-in)** — audio-only files (mp3, flac, opus, m4a, aac, wav, ogg, oga) are recognised when the operator sets `audio_types <list>` in the Caddyfile (empty by default to preserve 1.0.x behaviour). Audio files render as tiles with an inline SVG speaker-icon placeholder (Material Design `volume_up` glyph, themed via `--accent` CSS variable) and a duration pill (e.g. "4:45"). The lightbox opens a native `<audio controls preload="metadata">` element plus a parallel META panel showing codec, sample rate, channels, channel layout, duration, and bitrate. Stream-level metadata is extracted via `ffprobe` (2 subprocess calls) and cached in `.webp.ameta` sidecars parallel to the video's `.webp.vmeta`. The default `audio_types` list is empty so existing 1.0.x configs are unaffected. Per the Q2 "video wins" rule, files whose extension is in BOTH `video_types` and `audio_types` (e.g. `.ogg`) classify as `KindVideo` at extension-only classification time. See [`docs/03i-feature-audio.md`](docs/03i-feature-audio.md) for the full architecture.
